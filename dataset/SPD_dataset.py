...
dataset_dir = os.path.join(os.path.dirname(os.path.realpath(__file__)), 'SPD/healthcare-dataset-stroke-data.csv')
...
class SPDDataset(Dataset):
  def __init__(
    ...
    self.feature_to_loc = {c:i for i,c in enumerate(self.dataset.columns)}
    for header_str in ["gender", "Residence_type", "ever_married", "work_type", "smoking_status"]:
      self.dataset[header_str] = self.dataset[header_str].astype('category').cat.codes
    self.dataset = self.dataset.to_numpy()
    self.dataset = np.array(self.dataset, dtype=float)

    train_size = int(len(self.dataset) * config.get('train_val_split', 0.8))
    ...
    train_dataset, val_test_dataset = torch.utils.data.random_split(self.dataset, [train_size, old_val_size]) 
    val_dataset, test_dataset = torch.utils.data.random_split(val_test_dataset, [new_val_size, test_size])
...
class SPDTrainDataset(SPDDataset):
  ...
  def redistribute_dataset(self, dataset, feature_name, feature_distribution, equalize_dataset):
    ...
    total = len(dataset)
    _, counts = np.unique(np.array(dataset[:, feature_index]), return_counts=True)

    assert len(counts) == len(feature_distribution)
    if equalize_dataset:
      total = np.min(counts)*len(counts)

    feature_distribution = np.array(feature_distribution) / sum(feature_distribution)
    intended_counts = total * feature_distribution
    feature_masks = [np.ma.masked_less(np.arange(count), intended_counts[i], copy=True).mask for i, count in enumerate(counts)]
...
def get_spd_datasets(dataset_dir=dataset_dir, config={}, verbose=False):
  ...
  train_dataloader = DataLoader(SPDTrainDataset(), batch_size=config.get('batch_size', 64), shuffle=config.get('shuffle', True))
  val_dataloader = DataLoader(val_dataset, batch_size=config.get('batch_size', 64), shuffle=config.get('shuffle', True))
  test_dataloader = DataLoader(test_dataset, batch_size=config.get('batch_size', 64), shuffle=config.get('shuffle', True))
  if verbose:
